...
class CoreRobot(implicit config: RobotConfig, clock: Clock) {
  ...
  val dashboard = Future {
    val dash = new FunkyDashboard
    implicit val system = ActorSystem("funky-dashboard")
    implicit val materializer = ActorMaterializer()
    implicit val routingSettings = RoutingSettings.default

    Http().bindAndHandle(Route.handlerFlow(dash.route), "0.0.0.0", 8080)

    dash
  }

  dashboard.foreach(_.datasetGroup("shooter-arm").addDataset(new TimeSeriesNumeric("Pot Angle")(
